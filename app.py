...
SPOT_WEATHER_DATA = {
    ...
    "尚仁沢": {
        ...
        "x_url": "", "fb_url": "", "insta_url": "", "blog_url": "http://sofield.jugem.jp/", "yt_url": "",
...
def build_settings_flex_message(fav_list):
    ...
    for i in range(0, len(fav_list), chunk_size):
        ...
        # ★ 各セルの最上段に「1番お気に入りに設定する」選択ボタンを配置
        rows.append({
        ...
        # ★ ボタンのflex比率を調整し、文字領域(3) : 矢印上(2) : 矢印下(2) : 削除(2) で潰れを防止
        for spot in chunk:
            rows.append({
    ...
    if spot_name in MEMORY_CACHE:
        ...
        if now - updated_time <= timedelta(hours=1):
            if isinstance(data, dict):
                ...
                if data.get("_version") != "settings_shortcut_v4":
                    return None
                ...
                try:
                    ...
                    if now - updated_time <= timedelta(hours=1):
                        ...
                        if isinstance(weather_data, dict):
                            ...
                            if weather_data.get("_version") != "settings_shortcut_v4":
                                return None
                            if not weekly or len(weekly) < 4 or weekly[0].get("temp_max") == "-":
                                return None
...
def save_cached_weather(spot_name, weather_data):
    now = datetime.now(timezone.utc)
    weather_data["_version"] = "settings_shortcut_v4"
